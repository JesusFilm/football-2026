...
- Codex settings (full auto, no sandbox)
...
def setup_codex_settings():
    """Configure Codex CLI with full auto approval and no sandbox."""
    ...
    if config_file.exists():
        print(f"[post_install] Codex config exists, skipping: {config_file}", file=sys.stderr)
        return

    config = """\
# Codex CLI configuration
# Full auto mode — never prompt for approval
approval_policy = "never"

# Disable sandbox inside devcontainer
sandbox = "none"
"""
