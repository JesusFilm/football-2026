...
import { formatViews } from "@/lib/map-utils";
...
export function CountryViewsSummary({
  ...
  const topCountry = countries[0]?.countryName ?? "-";
  ...
  return (
    <div className="mb-6 rounded-none border-0 bg-transparent p-0 md:rounded-[var(--radius-lg)] md:border md:border-line md:bg-[rgb(12_10_8_/_0.54)] md:p-5 md:backdrop-blur-md">
      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <SummaryMetric label="Top country" value={topCountry} />
        <SummaryMetric label="Total views" value={formatViews(totalViews)} />
        <SummaryMetric label="Countries" value={String(countries.length)} />
      </div>
