        ...
        {unavailable ? (
          ...
          <>
            <CountryViewsSummary
              ...
              totalViewsLabel={
                regionDisplayCode
                  ? t("regionCodeViews", { regionCode: regionDisplayCode })
                  : t("totalViews")
              }
            />
            {visible ? (
...
function SectionState({ label, body }: { label: string; body: string }) {
  return (
    <div className="rounded-none border-0 bg-transparent p-8 text-center text-fg-dim backdrop-blur-none md:rounded-[var(--radius-lg)] md:border md:border-line md:bg-[rgb(12_10_8_/_0.65)] md:backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:h-full lg:w-full lg:grid-rows-2">
          {Array.from({ length: 8 }).map((_, index) => (
            <div
              ...
              data-testid="region-skeleton-filter"
            />
          ))}
        </div>
      </div>
