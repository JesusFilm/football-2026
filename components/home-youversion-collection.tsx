...
import { YouVersionPlanCard } from "@/components/youversion-plan-card";
import {
  YOUVERSION_COLLECTION_URL,
  YOUVERSION_PLANS,
} from "@/lib/youversion-plans";

export function HomeYouVersionCollection() {
  const t = useTranslations("HomeYouVersionCollection");

  return (
    <section
      aria-labelledby="youversion-collection-heading"
    ...
    >
      ...
      <div className="mb-7 flex flex-col gap-4 sm:mb-9 sm:flex-row sm:items-end sm:justify-between sm:gap-8">
        <div className="max-w-[640px]">
          <span className="font-mono text-[11px] font-semibold tracking-[0.18em] text-accent uppercase">
            {t("eyebrow", { count: YOUVERSION_PLANS.length })}
          </span>
          <h2
        ...
        <a
          href={YOUVERSION_COLLECTION_URL}
          target="_blank"
      ...
      {/* Mobile: 2-col grid of all 4 plans + Open library CTA below. */}
      <div className="sm:hidden">
        <ul role="list" className="grid grid-cols-2 gap-4">
          {YOUVERSION_PLANS.map((plan) => (
            <li key={plan.id}>
              <YouVersionPlanCard plan={plan} />
            </li>
          ))}
        </ul>
        <div className="mt-6 flex justify-center">
          ...
          >
            {t("showMoreCta", { total: YOUVERSION_PLANS.length })}
            <svg
              width="14"
              height="14"
              viewBox="0 0 14 14"
